          ...
          <div class="inp-con">
            <label for="one1">
              ...
              <el-tooltip class="item" effect="dark" content="任务名称不能超过20字" placement="top-start">
                <input type="text" id="one1" class="inp" v-model="taskData.name" autocomplete="off" />
              </el-tooltip>
            </label>
          </div>
        </div>
...
export default {
  ...
  methods: {
      ...
      this.$http.api
        .get(
          window.BASEURL.testUrl1 +
            `parent-police/api/v1/faceLibrary?&pageNum=${1}&pageSize=${
              VM.pagesize
            }&libName=${VM.name}`
        )
        .then(function(res) {
    ...
    next() {
      ...
      let start = new Date(this.dateVal1).getTime();
      let end = new Date(this.dateVal2).getTime();
      if (start == end) {
        this.$message({
      ...
      this.$http.api
        ...
        .then(function(res) {
          if (res.data.code === 0) {
            VM.taskData.validStartTime = start;
            VM.taskData.validEndTime = end;
            VM.key1 = false;
            VM.key2 = true;
            this.$notify({
              title: "提示",
