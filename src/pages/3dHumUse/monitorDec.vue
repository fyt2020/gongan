            ...
            <template v-for="(item,index) in videoArr ">
              <li class="right__item" :key="index" @click="goTo(item.list[0])">
                ...
                <div class="right__mid">
                  ...
                  <p class="isLibrary__box" v-if="1">相似度:{{((item.list[0].possibility* 100).toFixed(2))}}%</p>
                </div>
...
export default {
  ...
  beforeDestroy() {
    ...
    window.clearInterval(VM.timer3);
  ...
  methods: {
      ...
      this.$http.api
        ...
        .then(res => {
          if (res.status == 200) {
            ...
            if (VM.percent != 100) {
              window.clearInterval(VM.timer3);
              VM.timer3 = window.setInterval(VM.getVideoResult(VM.Md5), 10000);
            } else {
              window.clearInterval(VM.timer3);
              VM.timer3 = null;
            }
    ...
    query1(data) {
      ...
      this.frameNum = 0;
      this.$http.api
        .get(window.BASEURL.testUrl2 + `/face-task-manager/api/v1/tasks/${data}`)
        .then(res => {
          let data1 = res.data;
          if (data1.code == 0) {
            ...
            if (status != 5) {
              window.clearInterval( VM.timer1 );
              VM.timer1 = window.setInterval( VM.query1(data), 1000 );
            } else {
              window.clearInterval( VM.timer1 );
    ...
    queryFather(data) {
      ...
      window.clearTimeout(VM.timer2);
      this.$http.api
        .get(window.BASEURL.testUrl2 + `/face-task-manager/api/v1/tasks?status=5&pid=${data}`)
        .then(res => {
          if (res.status == 200) {
            ...
            if (resData == 0) {
              ...
              realFrameNum = res.data.result.realFrameNum;
              realFrameNumTime = res.data.result.realFrameNumTime;
              list = res.data.result.list;
              len = list.length;
            }

            VM.percent = Math.floor((realFrameNum / VM.frameNum) * 100);
            
            if ( realFrameNum != VM.frameNum ) {
              window.clearTimeout(VM.timer2);
