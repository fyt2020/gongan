    ...
    <div class="device-wrapper">
      ...
      <div class="dec-list">
        <el-table :data="tableData" stripe style="width: 100%" highlight-current-row>
          ...
          <el-table-column prop="deviceNo" label="设备编号" height="40" align="center" width="180" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column prop="deviceVersion" label="设备型号" width="180" height="40" align="center" :show-overflow-tooltip='true'>
            <template slot-scope="scope">
              <span>{{scope.row.deviceVersion == "0" ? "雪山三号33" : "雪山三号15" }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="orgName" label="所属单位" width="180" height="40" align="center" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column prop="address" label="所在场地" width="180" height="40" align="center" :show-overflow-tooltip='true'></el-table-column>
          <el-table-column
          ...
          ></el-table-column>
          <el-table-column prop="cRunStatus" label="运行情况" width="180" height="40" align="center" :show-overflow-tooltip='true'>
            <template slot-scope="scope">
              <span>{{scope.row.cRunStatus == '正常'  ? "正常" : scope.row.cRunStatus}}</span>
            </template>
          </el-table-column>

          <el-table-column prop="placeTag" label="控场" width="180" height="40" align="center" :show-overflow-tooltip='true'>
            <template slot-scope="scope">
          ...
          <el-table-column fixed="right" label="操作" width="180" height="40" align="center">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="wactch(scope.row)" class="btn">查看</el-button>
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item class="drop" @click.native="take(scope.row)" v-if="scope.row.deviceStatusCname == '启用'&& scope.row.cRunStatus == '正常'" >拍摄</el-dropdown-item>
                  <el-dropdown-item @click.native="handleDelete(scope.row)" class="drop">删除</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    beforeUploadFile(data, e) {
      ...
      VM.$http.api
        .put(window.BASEURL.oss+`parameters/`+ e.target.files[0].name, e.target.files[0], {
    ...
    deleteItem(data) {
      let VM = this;
      this.$http.api
        .delete(window.BASEURL.testUrl1 + `parent-police/api/v1/devices/${data}`)
        .then(function(res) {
          if (res.data.code === 0) {
    ...
    getDevicesList() {
      let VM = this;
      this.$http.api
        .get(window.BASEURL.testUrl1 + `parent-police/api/v1/devices?deviceNo=${VM.devSno}&orgId=${VM.orgId}&deviceVersion=${VM.deviceVersion}&pageNum=${this.currentPage}&pageSize=${this.pagesize}`)
        .then(function(res) {
          if (res.data.code === 0) {
